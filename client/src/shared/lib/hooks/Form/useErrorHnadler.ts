...
export const createErrorHandler = (setError: SetErrorFunction): (errorCode: ErrorCodes) => void => {
  ...
  return (errorCode: ErrorCodes) => {
    const message = errorMessages[errorCode] || 'An unexpected error occurred. Pleasetry again.';
