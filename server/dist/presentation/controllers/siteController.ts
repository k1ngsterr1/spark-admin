...
class EditController{
    ...
    async addSiteData(req: Request, res: Response): Promise<void>{
        ...
        try{
            ...
            if(errors.length > 0){
                ...
                res.status(current_errors.code).json({ message: current_errors.details });
                return;
            }

            res.status(200).json({ message: "Успешно добавлена компонента." });
        }catch(error){
            console.log(error);
    ...
    async updateSite(req: Request, res: Response): Promise<void>{
        ...
        try{
            ...
            await this.updateSiteComponent.execute(siteName, componentId, newValue, errors);
            ...
            console.log(error);
            res.status(500).json( {message: "Ошибка при обновление AgroPV" });
        }
    }
