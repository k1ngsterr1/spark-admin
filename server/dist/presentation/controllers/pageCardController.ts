...
import {
  AddBlockRequest,
  NewPageCardRequest,
} from "@core/utils/PageCard/Reqeust";
...
class PageCardController {
  ...
  async addPageCard(req: Request, res: Response): Promise<void> {
    ...
    try {
      ...
      console.log(error);
      res.status(500).json({
        message: "Произошла ошибка при добавление шаблонной карточки.",
      });
    }
  }
  ...
  async addBlock(req: Request, res: Response): Promise<void> {
    ...
    try {
      ...
      res
        .status(500)
        .json({ message: "Произошла ошибка при соединения с блоком." });
    }
  }
  ...
  async businessLanding(req: Request, res: Response): Promise<void> {
    ...
    try {
      ...
      console.log(data);
      await res.render("layouts/business-landing-template/main", {
